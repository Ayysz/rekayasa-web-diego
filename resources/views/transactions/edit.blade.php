...
<div class="bg-white shadow rounded-lg p-6 max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Edit Transaksi Pemesanan</h2>
    ...
    <form action="{{ route('admin.transactions.update', $transaction->id_transaksi) }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <input type="text" name="nama_pelanggan" id="nama_pelanggan" value="{{ old('nama_pelanggan', $transaction->nama_pelanggan) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500 sm:text-sm border p-2.5" required>
        </div>
        ...
        <div class="mb-5">
            ...
            <select name="id_dessert" id="id_dessert" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500 sm:text-sm border p-2.5" required>
                ...
                @foreach($desserts as $dessert)
                    <option value="{{ $dessert->id_dessert }}" {{ old('id_dessert', $transaction->id_dessert) == $dessert->id_dessert ? 'selected' : '' }}>
        ...
        <div class="mb-5">
            ...
            <input type="number" name="jumlah" id="jumlah" min="1" value="{{ old('jumlah', $transaction->jumlah) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500 sm:text-sm border p-2.5" required>
        </div>

        <div class="mb-5">
            <label for="tanggal_pesanan" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Pesanan</label>
            <input type="date" name="tanggal_pesanan" id="tanggal_pesanan" value="{{ old('tanggal_pesanan', $transaction->tanggal_pesanan) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500 sm:text-sm border p-2.5" required>
        </div>
